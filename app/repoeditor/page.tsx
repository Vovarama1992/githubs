  ...
  return (
    <div className={styles.card}>
      ...
      <div className={styles.cardBody}>
        ...
        <div className={styles.cardElement}>Last Commit: {new Date(repo.lastCommit).toLocaleDateString()}</div>
        <div className={styles.cardElement}>Owner: <a href={repo.owner.url} target="_blank" rel="noopener noreferrer">{repo.owner.name}</a></div>
        <div className={styles.cardElement}>
          {repo.owner.avatarUrl && <img src={repo.owner.avatarUrl} alt={repo.owner.name} className={styles.avatar} />}
        </div>
        <div className={styles.cardElement}>Languages: {repo.languages.join(', ')}</div>
        <div className={styles.cardElement}>Description: {repo.description}</div>
      </div>
      <div className={styles.cardFooter}>
      <Link href={`/?username=${encodeURIComponent(username)}&query=${encodeURIComponent(query)}`}>
